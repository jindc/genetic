...
def buildhiddenset(num=200):
    rows=[]
    for i in range(num):
        x=randint(0,40)
        y=randint(0,40)
        rows+=[ [x,y,hiddenfunction(x,y)  ] ]
    return rows
...
if __name__=='__main__':
    etree=exampletree()
    etree.display()
    print etree.evaluate([2,3])
    print etree.evaluate([5,3])    
    rtree=makerandomtree(2)
    rtree.display()
    print "r1",rtree.evaluate([2,4])

    rtree=makerandomtree(2)
    rtree.display()
    print "r2",rtree.evaluate([2,4])
    
    hiddenset=buildhiddenset()
    print hiddenset[0] 
    
    print scorefunction(rtree,hiddenset)
    rtree.display()
    rtree1=mutate(rtree,2)
    rtree1.display()
    rtree2=crossover(rtree,rtree1)
    rtree2.display()
    
    rf=getrankfunction(buildhiddenset())
    #ret = evolve(2,500,rf,mutationrate=0.2
    #    ,breedingrate=0.1,pexp=0.3,pnew=0.2)
    #print ret
    ...
    winner=evolve(5,20,tournament,maxgen=50) 
    print winner
